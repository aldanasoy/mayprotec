...
### Sprint 2: correcciones post-revisión de cliente (05-sep-2026)

Contexto: Sergio revisó el sprint de materiales/garantías en vivo y reportó 6 problemas. Investigación (2 agentes de exploración + verificación directa con DevTools en producción) confirmó los 6. Sitio hermano Ideatecny actualizado en paralelo por otro agente, no tocado aquí.

**Nota de coordinación:** este sprint corrió en paralelo con el sprint de "Agente de voz Mallas Colombia" (ver arriba) en la misma sesión de otro agente. Los cambios de este sprint a `LeadModal.astro` y `ContactForm.astro` (solo el ícono de WhatsApp) terminaron incluidos dentro del commit `32da90c` de esa otra sesión al coincidir en el tiempo — contenido verificado íntegro, sin pérdida de cambios, solo atribución mezclada en ese commit puntual.

- **Keyword "seguridad" agregada** en title/H1 de home, H1 de gatos, y title+H1 de las 3 páginas de servicio nuevas (canchas deportivas, protección comercial, antipalomas) — ninguna de las 3 nuevas la tenía, y la de protección comercial no tenía "seguridad" en absoluto en toda la página (solo se tocó title/H1, el cuerpo sigue usando "protección"/"cerramiento").
- **Corrupción "sí" revertida** (heredada de un commit viejo, 21-jul-2026, que reemplazó a ciegas la letra "s" por "sí"): variable de `.map()` en `Process.astro` (`sí` → `s`, 5 referencias), parámetro roto de `SearchAction` en el JSON-LD de `BaseLayout.astro` (`?sí=` → `?s=`), y tildes faltantes en `LeadModal.astro` (6 sitios), `Testimonials.astro` ("resenas"→"reseñas"), `gracias.astro`, `galeria.astro` (×3 "balcon"→"balcón"). Grep de barrido final confirmó solo 3 sobrevivientes legítimos (usos reales de la palabra "sí"/"así").
- **Fix raíz del fondo verde bajo fotos de gatos/perros:** `src/styles/global.css` — `img { max-width:100%; height:auto; }` estaba sin `@layer`, ganándole (por reglas de CSS Cascade Layers) a la utilidad `.h-full` de Tailwind usada en `ServiceCard.astro`. Confirmado en vivo con DevTools (245.9px real vs. contenedor de 280.5px). Envuelta en `@layer base`; además `ServiceCard.astro` reforzado con `position:absolute; inset:0` (mismo patrón que ya usa Ideatecny) para quedar inmune aunque el `@layer` se revierta a futuro.
- **3 carruseles nuevos desde cero** (no existía ningún patrón de scroll horizontal en este repo): grid de 11 tarjetas de `servicios.astro`, las 2 filas "Por espacio"/"Por necesidad" de `InstallTypes.astro`, y "Trabajos realizados" de `Gallery.astro` — mismo patrón Tailwind (`overflow-x-auto snap-x snap-mandatory` + `snap-start flex-shrink-0`) ya probado en el sitio hermano Ideatecny, sin librería nueva. Con botones prev/next en desktop.
- **Logo oficial de WhatsApp:** descargado (con permiso explícito) desde Wikimedia Commons a `public/images/whatsapp-logo.svg`, reemplazando los SVG dibujados a mano en las 10 ubicaciones existentes (ya eran consistentes entre sí, solo se cambió el asset).
- **Barra inferior verde nueva — `src/components/layout/BottomBar.astro` (componente NUEVO, no una restauración):** verificado exhaustivamente que nunca existió en este repo (0 resultados en `git log --all` de las 61 revisiones previas, todas las ramas). Sergio la recordaba probablemente del sitio hermano Ideatecny (que sí la tiene, oscura no verde). Construida con el mismo patrón (mobile-only, auto-hide con teclado), tema verde de marca (`--color-brand`), botones WhatsApp + "Llamar ahora" leyendo el teléfono de `GBP.phone` (no un 5º hardcodeo). `WhatsAppFloat` ocultado en mobile (`hidden md:block`) para no duplicar.
- **Pendiente — tracking GTM/GA4 de llamadas (tarea separada, fuera de este codebase):** el botón "Llamar ahora" ya es un `<a href="tel:...">` real, listo para que un trigger genérico de GTM lo detecte. Falta configurar en el contenedor propio de MayProtec (`GTM-TCV97SLJ`) el mismo tag+trigger "Click - tel links" que ya funciona en el contenedor de Ideatecny (`GTM-MBSWSVLZ`) — vía API de GTM, usando `scripts/setup-gtm*.cjs` ya existentes en este repo, adaptando el `containerId`. No hecho en este sprint.

**Verificación:** `astro build` → 20 páginas, sin errores (mismo conteo). Confirmado visualmente (dev server + mediciones DOM) el fix del fondo verde y el comportamiento de la barra inferior en mobile.
